...
# --- 1. CONFIGURACIÓN ---
st.set_page_config(page_title="ARGOS v6.0 - Full Terminal", layout="wide")
...
def analyze_asset(ticker):
    try:
        ...
        # R2 Dinámico (Fuerza de la Ineficiencia)
        r2_series = []
...
with tab2:
    st.subheader("🎯 Ejecución Pro (Niveles Matemáticos)")
    ...
    if de:
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.markdown(f'<div class="tp-card">Objetivo Diario (EMA 21): {de["ema"]:.5f}</div>', unsafe_allow_html=True)
...
with tab3:
    st.subheader("🎲 Análisis Montecarlo Profundo")
    target_m = st.selectbox("Simular Probabilidades:", ASSETS, key="mc_s")
    ...
    if dm:
        ...
        prob_exito = (caminos[:, -1] < dm['price']).sum() / sims * 100 if dm['z'] > 0 else (caminos[:, -1] > dm['price']).sum() / sims * 100
        st.metric("Probabilidad de éxito de la señal", f"{prob_exito:.1f}%")

with tab4:
    st.subheader("🛡️ Sentinel Macro")
    try:
        s_df = yf.download(['^GSPC', '^VIX', 'DX-Y.NYB'], period='5d', progress=False)['Close']
        c1, c2, c3 = st.columns(3)
        c1.metric("S&P 500", f"{float(s_df['^GSPC'].iloc[-1]):.2f}")
        c2.metric("VIX", f"{float(s_df['^VIX'].iloc[-1]):.2f}")
        c3.metric("DXY", f"{float(s_df['DX-Y.NYB'].iloc[-1]):.2f}")
    except: st.warning("Datos macro en pausa.")
...
with tab6:
    ...
    with col_b2:
        ...
        # Generación dinámica de spreads para todos los pares
        yield_data = {'US10Y': 4.25, 'GER10Y': 2.40, 'UK10Y': 4.10, 'CAN10Y': 3.50, 'AUS10Y': 4.40, 'JPN10Y': 0.85}
        spreads = {
            "EUR/USD": yield_data['GER10Y'] - yield_data['US10Y'],
            "GBP/USD": yield_data['UK10Y'] - yield_data['US10Y'],
            "CAD/USD": yield_data['CAN10Y'] - yield_data['US10Y'],
            "AUD/USD": yield_data['AUS10Y'] - yield_data['US10Y'],
            "JPY/USD": yield_data['JPN10Y'] - yield_data['US10Y']
        }
